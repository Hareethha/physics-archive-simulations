...
function setPlayButton(){
  ...
  document.querySelectorAll('.local-play').forEach(btn=>{
    const isActive = btn.dataset.tab === activeTab;
    btn.textContent = (playing && isActive)
      ? '❚❚ Pause motion'
      : (btn.dataset.tab === 'graphs' ? '▶ Play graphs' : '▶ Play motion');
    btn.classList.toggle('is-playing', playing && isActive);
    btn.classList.toggle('primary', !(playing && isActive));
  });
}
...
document.querySelectorAll('.local-play').forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    const max=parseFloat(slider.max)||1;
    if(parseFloat(slider.value)>=max-1e-6){
      slider.value=parseFloat(slider.min)||0;
      slider.dispatchEvent(new Event('input',{bubbles:true}));
    }
    playing=true;
    lastAnimationStamp=null;
...
document.querySelectorAll('.local-restart').forEach(btn=>{
  btn.addEventListener('click',()=>{
