...
function setTab(name){tabs.forEach(b=>b.classList.toggle('active',b.dataset.tab===name));panes.forEach(p=>p.classList.toggle('hidden',p.id!==name));updateAll();}
...
function drawAxes(ctx,W,H,m,xmin,xmax,ymin,ymax,xlab,ylab){
  ...
  ctx.fillStyle='#664151';ctx.font='13px Trebuchet MS';ctx.fillText(xlab,W-45,H-12);ctx.save();ctx.translate(20,H/2);ctx.rotate(-Math.PI/2);ctx.fillText(ylab,0,0);ctx.restore();
  return {X,Y};
}
...
function updateAll(){drawIndependent();drawBuilder();drawHorizontal();drawAngle();drawUnequal();drawGraphs();}
updateAll();
